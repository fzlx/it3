(function () {
    ...
    var loadDemoSettings = function () {
        ...
        if (storageSupported) {
            ...
            try {
                ...
                result.turbo_mode = (window.localStorage.demo_turbo_mode && window.localStorage.demo_turbo_mode === '1');
                window.localStorage.demo_fixed_navbar != undefined && (result.fixed_navbar = (window.localStorage.demo_fixed_navbar && window.localStorage.demo_fixed_navbar === '1'));
                window.localStorage.demo_fixed_menu != undefined && (result.fixed_menu = (window.localStorage.demo_fixed_menu && window.localStorage.demo_fixed_menu === '1'));
                result.rtl = (window.localStorage.demo_rtl && window.localStorage.demo_rtl === '1');
                window.localStorage.demo_footer_fixed != undefined && (result.footer_fixed = (window.localStorage.demo_footer_fixed && window.localStorage.demo_footer_fixed === '1'));
                window.localStorage.demo_position_fixed != undefined && (result.position_fixed = (window.localStorage.demo_position_fixed && window.localStorage.demo_position_fixed === '1'));
        ...
        for (var i = 0, l = demo_cookies.length; i < l; i++) {
            ...
            val = demo_cookies[i].substr(pos + 1).replace(/^\s+|\s+$/g, '');
             ...
             if (key === 'demo_turbo_mode') {
                result.turbo_mode = (val === '1') ? true : false;

            } else if (key === 'demo_fixed_navbar') {
    ...
    var saveDemoSettings = function () {
        if (storageSupported) {
            try {
                window.localStorage.demo_turbo_mode = escape((demo_settings.turbo_mode) ? '1' : '0');
        ...
        document.cookie = 'demo_turbo_mode=' + escape((demo_settings.turbo_mode) ? '1' : '0');
    ...
    $(function () {
        ...
        var demo_template = [
            ...
            '     <li class="clearfix">',
            '       <span>极速模式</span>',
            '       <div class="demo-checkbox"><input type="checkbox" id="demo-turbo-mode" class="demo-settings-switcher" data-class="switcher-sm"' + ((demo_settings.turbo_mode) ? ' checked="checked"' : '' ) + '></div>',
            '     </li>',
            '     <li class="clearfix">',
            '       <span>固定导航条</span>',
...
//            '     </li>',
        ...
        $(function () {
            ...
            $('#content-wrapper').append($(demo_template.join('\n')));
			...
			// Fix/unfix turbo mode
            $('#demo-turbo-mode').on($('html').hasClass('ie8') ? "propertychange" : "change", function () {
                var uncheck_turbo_chk = (!$(this).is(':checked') && $('#demo-turbo-mode').is(':checked')) ? true : false;
                demo_settings.turbo_mode = $(this).is(':checked');
                saveDemoSettings();
                reloadPage();
            });
            
            // Fix/unfix turbo navbar
