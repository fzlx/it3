(function(w) {
	...
	var callback = function(resp, fnCallback, failure) {
		if (resp.successful) {
			...
			layer.alert('您当前还未登录，点击确定后重新登录！',function(){
				top.location.href = rootAdminPath + "/login";
			});
		} else {
	...
	var exception = function(xhr, textStatus, errorThrown){
		if(xhr.status == 500){
			top.layer.open({
				type: 1,
		  		title: "异常提示",
		  		area: ['70%', '80%'],
		  		content: '<div style="padding: 20px; word-break:break-all; word-wrap:break-word;">' + 
	...
	var call = function(ajaxType, _url, _param, _success, method, refBy, async, failure, isShowTips) {
		...
		setting.error = function(xhr, textStatus, errorThrown) {
			...
			exception(xhr, textStatus, errorThrown);
		};
		...
		$.ajax({
			...
			beforeSend: function() {
		  	...
		  	error: function(xhr, textStatus, errorThrown) {
		  		dom.html('<div class="ajax-loading-error">' + xhr.responseText.split('\n').join('<br/>') + '</div>');
		  	}
		})
	};
