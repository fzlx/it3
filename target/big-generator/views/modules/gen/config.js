	...
	var configSvc = {
			...
			inputStrFun: function(info){//文本框字符串
				...
				if("radio"==info.inputType||"checkbox"==info.inputType){
					...
					var remarks = info.remarks?info.remarks:"";
					returnStr ='<input type="text" class="m-wrap ' +(info.isRequired=="1"? "required" : "")+ '" ' +isRequired+'  value="'+info.configKeyValue+'" name="'+info.configKey+'" />'+'<span style="color:red">&nbsp;'+remarks+'</span>';
				}else if("select"==info.inputType){
					returnStr = '<select class="m-wrap" name="'+info.configKey+'" '+isRequired+' style="width:50%">'
					var inputTextArray = info["inputText"].split(",");
					var inputValueArray = info["inputValue"].split(",");
					var checkFlag = "";//是否选中
					for(var i=0;i<inputTextArray.length;i++){
					...
					returnStr = '<textarea class="m-wrap span12" '+isRequired+' name="'+info.configKey+'" style="width:50%;height:100px;">'+info.configKeyValue+'</textarea>';
